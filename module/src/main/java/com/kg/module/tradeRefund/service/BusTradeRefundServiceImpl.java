...
public class BusTradeRefundServiceImpl extends ServiceImpl<BusTradeRefundMapper, BusTradeRefund> implements BusTradeRefundService {
    ...
    public String exportExcel(String params) {
        try {
            ...
            List<BusTradeRefundExcelOutDTO> dataList = list.stream()
                    .map(d -> JSONUtil.toBean(JSONUtil.parseObj(d, true), BusTradeRefundExcelOutDTO.class))
            ...
            // 写入导出excel文件
            ExcelWriteUtils.write(path, title, dataList, BusTradeRefundExcelConstant.EXPORT_EXCEL_COLUMN);
            // 导出成功，返回导出地址
    ...
    public void importExcel(HttpServletRequest request) {
        ...
        List<BusTradeRefund> importData =
                ExcelReadUtils.read(request, 1, 2, BusTradeRefund.class, BusTradeRefundExcelConstant.IMPORT_EXCEL_COLUMN);
        // 处理数据
