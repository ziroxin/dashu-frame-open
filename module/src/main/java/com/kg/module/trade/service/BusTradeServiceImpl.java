...
public class BusTradeServiceImpl extends ServiceImpl<BusTradeMapper, BusTrade> implements BusTradeService {
    ...
    public String exportExcel(String params) {
        try {
            ...
            List<BusTradeExcelOutDTO> dataList = list.stream()
                    .map(d -> JSONUtil.toBean(JSONUtil.parseObj(d, true), BusTradeExcelOutDTO.class))
            ...
            // 写入导出excel文件
            ExcelWriteUtils.write(path, title, dataList, BusTradeExcelConstant.EXPORT_EXCEL_COLUMN);
            // 导出成功，返回导出地址
    ...
    public void importExcel(HttpServletRequest request) {
        // 读取导入数据
        List<BusTrade> importData = ExcelReadUtils.read(request, 1, 2, BusTrade.class, BusTradeExcelConstant.IMPORT_EXCEL_COLUMN);
        // 处理数据
