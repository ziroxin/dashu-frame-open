      ...
      for (let i = 0; i < chunks; i++) {
        ...
        await new Promise(async (resolve, reject) => {
          await this.getChunkFileMd5(chunkFile, async md5 => {
            const params = {chunkMD5: md5, chunk: i, chunks: chunks, name: newFile.name, uploadId: uploadId}
            await this.$request({
              ...
              if (response.code === '200') {
                if (response.data !== null) {
                  ...
                  // 分片不存在，新上传，更新状态
                  let data = {chunk: i, chunks: chunks, name: newFile.name, file: chunkFile, uploadId: uploadId}
                  await this.$request({
