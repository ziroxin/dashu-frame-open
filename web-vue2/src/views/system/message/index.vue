	...
	<div class="app-container">
		...
		<div style="margin-bottom: 10px;">
			<el-input v-model="searchData.msgId" size="small" style="width: 150px;margin-right: 10px;"
					  		class="filter-item" placeholder="请输入消息中心id查询"/>
			<el-input v-model="searchData.msgTitle" size="small" style="width: 150px;margin-right: 10px;"
					  		class="filter-item" placeholder="请输入消息标题查询"/>
			<el-input v-model="searchData.msgContent" size="small" style="width: 150px;margin-right: 10px;"
					  		class="filter-item" placeholder="请输入消息内容查询"/>
			<el-input v-model="searchData.msgRouter" size="small" style="width: 150px;margin-right: 10px;"
					  		class="filter-item" placeholder="请输入跳转路由查询"/>
			...
			<el-input v-model="searchData.msgType" size="small" style="width: 150px;margin-right: 10px;"
					  		class="filter-item" placeholder="请输入消息类型（0私信1公开）查询"/>
			<el-input v-model="searchData.permissionName" size="small" style="width: 150px;margin-right: 10px;"
					  		class="filter-item" placeholder="请输入所属模块/菜单（可为空）查询"/>
			<el-date-picker v-model="searchData.createTime" size="small" style="width: 150px;margin-right: 10px;"
							type="datetime" class="filter-item" placeholder="请选择消息创建时间查询"/>
			...
			<div style="float: right;">
				<el-button v-waves type="primary" icon="el-icon-plus" @click="openAdd" size="small"
                   v-permission="'message-zMessage-add'">新增
				</el-button>
				<el-button v-waves type="info" icon="el-icon-edit" @click="openUpdate(null)" size="small"
                   v-permission="'message-zMessage-update'">修改
				</el-button>
				<el-button v-waves type="danger" icon="el-icon-delete" @click="deleteByIds(null)" size="small"
                   v-permission="'message-zMessage-delete'">删除
				</el-button>
				<el-upload v-permission="'message-zMessage-importExcel'" style="display: inline-block;margin: 0px 10px;"
						   :action="$baseServer+'/message/zMessage/import/excel'" :headers="$store.getters.headerToken"
				...
				<el-button v-waves type="success" icon="el-icon-printer" @click="exportExcel" size="small"
						   v-permission="'message-zMessage-exportExcel'">导出Excel
				</el-button>
			</div>
		</div>
		<!-- 消息中心-列表 -->
		<el-table ref="dataTable" :data="tableData" stripe border @selection-change="handleTableSelectChange" v-loading="isLoading">
			...
			<el-table-column label="消息中心id" prop="msgId" align="center"/>
			<el-table-column label="消息标题" prop="msgTitle" align="center"/>
			<el-table-column label="消息内容" prop="msgContent" align="center"/>
			<el-table-column label="跳转路由" prop="msgRouter" align="center"/>
			<el-table-column label="消息状态（0未读1已读）" prop="msgStatus" align="center"/>
			<el-table-column label="已读时间" prop="readTime" align="center"/>
			<el-table-column label="消息类型（0私信1公开）" prop="msgType" align="center"/>
			<el-table-column label="所属模块/菜单（可为空）" prop="permissionName" align="center"/>
			<el-table-column label="消息创建时间" prop="createTime" align="center"/>
			<el-table-column fixed="right" label="操作" width="120" align="center">
				<template v-slot="scope">
					<el-button type="text" style="color: #13ce66;"
							   size="small" @click="openView(scope.row)">详情</el-button>
					<el-button v-permission="'message-zMessage-update'"
		...
		<el-dialog :title="titleMap[dialogType]" :close-on-click-modal="dialogType !== 'view' ? false : true"
			...
			<el-form ref="dataForm" :model="temp" label-position="right" label-width="100px" :disabled="dialogType==='view'">
				<el-form-item label="消息中心id" prop="msgId"
                      :rules="[{required: true, message: '消息中心id不能为空'}]">
					<el-input v-model="temp.msgId" placeholder="请输入消息中心id"/>
				</el-form-item>
				...
				<el-form-item label="消息内容" prop="msgContent"
                      :rules="[]">
					<el-input v-model="temp.msgContent" type="textarea" maxlength="1,000"
                    placeholder="请输入消息内容"/>
				</el-form-item>
				<el-form-item label="跳转路由" prop="msgRouter"
                      :rules="[]">
					<el-input v-model="temp.msgRouter" placeholder="请输入跳转路由"/>
				</el-form-item>
				...
				<el-form-item label="消息类型（0私信1公开）" prop="msgType"
                      :rules="[{required: true, message: '消息类型（0私信1公开）不能为空'}]">
					<el-input v-model="temp.msgType" placeholder="请输入消息类型（0私信1公开）"/>
				</el-form-item>
				<el-form-item label="所属模块/菜单（可为空）" prop="permissionName"
                      :rules="[]">
					<el-input v-model="temp.permissionName" placeholder="请输入所属模块/菜单（可为空）"/>
				</el-form-item>
			</el-form>
