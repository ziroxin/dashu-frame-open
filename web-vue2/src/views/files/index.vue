<template>
	<div class="app-container">
		...
		<el-table ref="dataTable" :data="tableData" stripe border @selection-change="handleTableSelectChange" v-loading="isLoading">
			...
			<el-table-column label="文件地址（文件访问地址）" prop="fileUrl" align="center"/>
			<el-table-column label="原文件名" prop="fileOldName" align="center"/>
			<el-table-column label="存储文件名" prop="fileName" align="center"/>
			<el-table-column label="文件扩展名" prop="fileExtend" align="center"/>
...
export default {
  ...
  methods: {
    ...
    loadTableList() {
      ...
      request({
        url: '/files/zFiles/list', method: 'get', params
      }).then((response) => {
        const {data} = response
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          if (this.dialogType === 'update') {
            ...
            request({
              url: '/files/zFiles/add', method: 'post', data
            }).then(response => {
