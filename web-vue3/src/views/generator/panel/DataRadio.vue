      ...
      <el-tab-pane label="静态" name="static">
        <template v-if="'static'===current.dataType">
          <el-form-item label="数据" label-width="60px" class="mt-10px">
            <vue-draggable v-model="current.radioData" :animation="150"
                           :key="'drag'+current.radioData.length">
              <div v-for="(item,idx) in current.radioData" :key="idx" class="flex items-center mb-10px">
                ...
                <base-button type="danger" link icon="el-icon-delete" class="mr-5px" size="small"
                             @click="()=>{current.radioData.splice(idx,1)}"/>
              </div>
            </vue-draggable>
...
const addStaticData = () => {
  const obj = cloneDeep(current.value.radioData)
  current.value.radioData = []
  current.value.radioData = [...obj, {value: 'value' + (obj.length + 1), label: 'label' + (obj.length + 1)}]
}
const refreshDynamicData = () => {
  ...
  request({url: obj.url, method: 'get'}).then((response) => {
    current.value.radioData = obj.dataKey.split('.').reduce((acc, k) => acc[k], response) || []
  })
}
const refreshDictData = () => {
  ...
  current.value.radioData = getDict(current.value.dictCode)
}
</script>
