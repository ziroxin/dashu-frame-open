...
const getList = () => {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    list.push({width, height, id: toAnyString(), image_uri: `https://dummyimage.com/${width}x${height}`})
  }
...
<template>
  <ContentWrap :title="t('router.waterfall')">
    <Waterfall :data="data" :loading="loading" :end="end"
               :props="{src: 'image_uri',height: 'height'}" @load-more="loadMore"/>
  </ContentWrap>
</template>
