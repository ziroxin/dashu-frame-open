...
const getUserAccessSource = async () => {
  ...
  if (res) {
    ...
    pieOptionsData!.series![0].data = res.data.map((v) => {
      return {name: t(v.name), value: v.value}
    })
  }
}
...
const getWeeklyUserActivity = async () => {
  ...
  if (res) {
    ...
    set(barOptionsData, 'series', [{
      name: t('analysis.activeQuantity'), data: res.data.map((v) => v.value), type: 'bar'
    }])
  }
}
...
const getMonthlySales = async () => {
  ...
  if (res) {
    ...
    set(lineOptionsData, 'series', [{
      name: t('analysis.estimate'), smooth: true, type: 'line', data: res.data.map((v) => v.estimate),
      animationDuration: 2800, animationEasing: 'cubicInOut'
    }, {
      name: t('analysis.actual'), smooth: true, type: 'line', itemStyle: {}, data: res.data.map((v) => v.actual),
