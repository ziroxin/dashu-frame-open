...
function requestCheckRepeatSubmit(config) {
  ...
  if (!skipRepeatSubmitCheck) {
    ...
    try {
      ...
      const o1 = JSON.parse(sessionStorage.getItem(storageKeys.s_oldReqObj) as string)
      ...
      // console.log('获取上一次请求对象失败：', e)
    }
    // 保存本次请求对象到sessionStorage
    sessionStorage.setItem(storageKeys.s_oldReqObj, JSON.stringify(reqObj))
  }
}

// 请求参数加密处理
function requestCheckEncrypt(config) {
  const isFormData = config.headers.getContentType() && config.headers.getContentType().toLowerCase().indexOf('multipart/form-data') >= 0
  if (isFormData) {
  ...
  try {
    ...
    if (config.url.indexOf('?') > -1) {
      ...
      const paramsArr = config.url.split('?')[1].split('&')
  ...
  response => {
    ...
    if (res.code === '200') {
      ...
      if (notLoginError(res.code)) {
        ...
        const errMsg = res.message || logErr
